...
export function runDLA(ctx, canvasWidth, canvasHeight, seedX, seedY) {
  ...
  function generateDlaInfluencedColor(neighborColors) {
    ...
    if (neighborColors.length >= 2) {
      factor = 1 / Math.pow(neighborColors.length, 0.17);
    }
    return {
      r: Math.min(255, Math.max(0, avg.r + (Math.random() - 0.5) * variation * 2)) * factor,
      g: Math.min(255, Math.max(0, avg.g + (Math.random() - 0.5) * variation * 2)) * factor,
      b: Math.min(255, Math.max(0, avg.b + (Math.random() - 0.5) * variation * 2)) * factor
    };
  }
  ...
  animationStep(); // Initial call to start the loop
