...
export function runColorFill(ctx, canvasWidth, canvasHeight, seedX, seedY) {
  ...
  const frontier = new Map(); // key = "x,y", value = [x, y]
  const frontierWeights = new Map(); // key = "x,y", value = weight
  ...
  function generateInfluencedColor(neighborColors) {
    const SIMILARITY_PERCENT = 0.9;
    const SIMILAR_VARIATION = 10;
    const DISSIMILAR_VARIATION = 25;
    ...
    if (neighborColors.length >= 4) { // Original logic: >= 7 or 8, adapted for potentially sparser starts
      useSimilar = true;
    } else {
  ...
  function pickFrontierPixel() {
    ...
    const weights = entries.map(([fx, fy]) => frontierWeights.get(`${fx},${fy}`) || 1);
    const total = weights.reduce((a, b) => a + b, 0);
    if (total === 0) return entries[0]; // Should not happen if frontier is not empty
  ...
  function step() {
    ...
    if (stepCount % 100 === 0 && frontier.size > 0) { // Only update weights if there's a frontier
      for (const [key, [fx, fy]] of frontier.entries()) {
