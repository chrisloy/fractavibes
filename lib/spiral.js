export function runSpiral(ctx, canvasWidth, canvasHeight, seedX, seedY) {
  ...
  function animationStep() {
    ...
    for (let i = 0; i < PIXELS_PER_ANIMATION_FRAME && currentPixelIndex < totalPixelsToPaint; i++) {
      ...
      const currentColor = hslToRgb(finalH, finalS, finalL);
      paintPixelToBuffer(px, py, currentColor);

      currentPixelIndex++;
      paintedThisFrameCount++;
    }

    if (paintedThisFrameCount > 0) {
    ...
    if (currentPixelIndex < totalPixelsToPaint) {
      ...
      console.log("Spiral fill finished: All pixels painted.");
      animationFrameId = null; 
    }
  }
  
  if (totalPixelsToPaint > 0) {
    // Paint the very first pixel with the initial random HSL values before starting the loop
    // This ensures that the random walk for the *second* pixel has a defined base.
    // Or, rely on the initial currentPaintH/S/L values for the first pixel in the loop if currentPixelIndex === 0.
    // The current logic handles the first pixel by not applying a random step if currentPixelIndex === 0.
    animationFrameId = requestAnimationFrame(animationStep);
  } else {
    console.log("Spiral fill: No pixels to paint (canvas 0x0 or invalid seed?).");
  }
